...
<%--template for pairs of tariff--%>
<div id="card-pair-template" style="margin-top: 20px; display: none;">
    <div class="card" id="card1" style="width: 30%;margin-left: 5%">
        <div class="card-body">
    ...
    <div class="card" id="card2" style="width: 30%; margin-left: 10%">
        <div class="card-body">
...
<%--<div id="contractForNT">--%>
    ...
    <%--<button style="border:1px solid gray; margin:0; padding:0 .6em; height:2.3em; box-sizing:border-box;"--%>
            <%--onclick="changeTariff($('#contract-selector').val(), tariffId)">Go</button>--%>
<%--</div>--%>
...
<script>
    ...
    function startTariffChange(tariffId) {
        console.log('Changing to' + tariffId);
    ...
    function changeTariff(contractId, tariffId){
        console.log( contractId + 'Add' + tariffId);
        $.ajax({
            type: "PUT",
            url: "/api/contracts/" + contractId +  "/tariff/" + tariffId,
    ...
    $(document).ready(function () {
        ...
        $.getJSON('/api/tariffs/active', function (data) {
            console.log(data);
            if (data.length === 0) {
                $('#tariffs-container').append('<h4>No tariffs available at the moment :(</h4>')
            } else {
                for (var i = 0; i < data.length; i += 2) {
                    ...
                    if (i + 1 < data.length) {
                        ...
                        cardPair.find('#desc1').html('\n');
                        cardPair.find('#btn1').attr('onclick', 'startTariffChange(' + data[i].id + ')');

                        cardPair.find('#title2').html(data[i + 1].name);
                        cardPair.find('#subtitle2').html(data[i + 1].cost + '$');
                        cardPair.find('#desc2').html('\n');
                        cardPair.find('#btn2').attr('onclick', 'startTariffChange(' + data[i + 1].id + ')');
                        ...
                        cardPair.find('#card2').css('display', 'none');
                    }

                    cardPair.attr('id', '#card-pair' + (i + 2))
                    $('#tariffs-container').append(cardPair);
                }
            }
        })
    })
