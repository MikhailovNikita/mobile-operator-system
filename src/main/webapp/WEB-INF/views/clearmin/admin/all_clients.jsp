...
<script>
    $(document).ready(function () {
        $.getJSON('/api/clients', function (response) {
            ...
            var table = $('<table id="table-body" style="width: 100%" class="table table-striped" cellspacing="0" cellpadding="0"/>'),
                thead = $('<thead/>'),
                th = $('<th/>'),
                tbody = $('<tbody/>'),
                tr = $('<tr/>'),
            ...
            thisTR.append(th.clone().html("Last name"));
            thisTR.append(th.clone().html("Email"));
            thisTR.append(th.clone().html("Passport"));
            thead.append(thisTR);
            table.append(thead);
